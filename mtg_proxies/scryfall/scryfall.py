...
def get_image(image_uri: str, *, silent: bool = False) -> str:
    ...
    if query.isdigit() and file_path.is_file() and file_path.stat().st_mtime < int(query):
        file_path.unlink()
        # Also remove cropped variants (e.g. stem_14_14.png) generated by the
        # PDF printer — they are derived from the base file and must be
        # regenerated when the base is re-downloaded.
        for variant in _cache_folder.glob(file_path.stem + "_*" + file_path.suffix):
            variant.unlink()
